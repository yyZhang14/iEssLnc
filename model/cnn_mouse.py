...
def get_data():
    # 读取数据集
    res = pd.read_csv("../result/mouse/para/300_40_32/dataset2.csv", header=0,
                      index_col=0)
    print(len(res))
...
def get_model():
    model = Sequential()
    model.add(Conv1D(filters = 32,kernel_size=3, strides=1,padding='valid',activation='relu',input_shape=(32,1)))
...
if __name__ == '__main__':
    ...
    for train_index, test_index in loo.split(data):
        ...
        x_train = x_train.reshape(x_train.shape[0], 32, 1).astype('float32')
        x_test = x_test.reshape(x_test.shape[0], 32, 1).astype('float32')
