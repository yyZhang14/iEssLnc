...
std::vector<long long> l_table(0), p_table(0);

void NodeType() {
//这个函数是 把lncRNA ,protein节点存储在数组中
  long long i;
  long long target;
  for (i = 0; i < table_size; i++) {
    ...
    if (vocab[target].word[0] == 'l') {
      l_table.push_back(target);
  ...
  std::cout << "l_table.size() " << l_table.size() << std::endl;
  std::cout << "p_table.size() " << p_table.size() << std::endl;
}
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) { //  
          if (d == 0) {
            ...
            if (pp) {
              if        (vocab[last_word].word[0] == 'l') {
                target = l_table[(next_random >> 16) % l_table.size()];
